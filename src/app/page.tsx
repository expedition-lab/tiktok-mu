...
function filterCreators(q: string, niche: string, minFollowers: number) {
...
function runTests() {
  ...
  try {
    ...
    assert(filterCreators("", "Travel", 0).every((c) => c.niche.includes("Travel")), "niche filter Travel");
    assert(filterCreators("", "all", 120000).every((c) => c.followers >= 120000), "minFollowers filter");
...
export default function App() {
  const [route, setRoute] = useState<{ name: "home" | "creator" | "live" | "dashboard"; id: string}>({
    name: "home",
    id: "",
  });
  ...
  const [booking, setBooking] = useState<null | { creator: any; pkg: any }>(null);
  ...
  const currentCreator = route.name !== "home" ? CREATORS.find((c) => c.id === route.id) : null;
    ...
    <div className="min-h-screen w-full bg-[#0e0f12] text-white">
      ...
      {route.name === "home" && (
        <HomeView
          ...
          openDashboard={() => setRoute({ name: "dashboard", id: "" })}
        />
      )}
...
function LivePage({ creator }: { creator: any }) {
        ...
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="mt-2 flex gap-2">
              ...
              />
              <button onClick={send} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-black hover:bg-white/90">
                Send
              </button>
            </div>
          </div>
      ...
      <div className="max-h-[90vh] w-[90vw] max-w-xl overflow-auto rounded-xl border border-white/10 bg-[#0e0f12] p-4" onClick={(e) => e.stopPropagation()}>
        ...
        {paymentFlow === "juice" && (
          <div className="mt-4 rounded-xl border border-white/10 bg-black/30 p-3">
            ...
            <input
              type="file"
              onChange={(e) => setProofFileName(e.target.files && e.target.files[0] ? e.target.files[0].name : "")}
        ...
        {paymentFlow === "myt" && (
          <div className="mt-4 rounded-xl border border-white/10 bg-black/30 p-3">
            ...
            <div className="mt-2 rounded-lg border border-white/10 bg-black/40 p-3 text-sm">
              ...
              <div className="mt-2 text-xs text-white/60">Demo link: {externalLink}</div>
            </div>
            <div className="mt-2 flex justify-between">
              <button onClick={() => setPaymentFlow("idle")} className="rounded-md px-3 py-2 text-sm hover:bg-white/10">Back</button>
...
function Dashboard({ onBack, extra = [] as any[] }: { onBack: () => void; extra?: any[] }) {
  const mockBookings = [
    {
      id: "b1",
      creator: "@aisha.mu",
